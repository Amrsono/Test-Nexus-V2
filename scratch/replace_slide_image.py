...
def main():
    ...
    if picture_shape:
        print(f"Replacing image in shape: {picture_shape.name}")
        with open(new_image_path, 'rb') as f:
            new_blob = f.read()
        
        # Overwrite the part's blob
        picture_shape.image.part._blob = new_blob
        
        prs.save(pptx_path)
        print("Image replaced successfully.")
    else:
        print("Picture shape not found on Slide 2.")
